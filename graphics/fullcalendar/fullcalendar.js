...
function Grid(element, options, methods) {
	...
	function renderGrid(r, c, colFormat, showNumbers) {
		...
		if (!tbody) { // first time, build all cells from scratch
			...
			tbody.find('td').bind('contextmenu',dayRightClick);
			...
			tbody.find('td.fc-new').removeClass('fc-new').bind('contextmenu',dayRightClick);
	...
	function dayRightClick(ev) {
		var n = parseInt(this.className.match(/fc\-day(\d+)/)[1]),
			date = addDays(
				cloneDate(view.visStart),
				Math.floor(n/colCnt) * 7 + n % colCnt
			);
		view.trigger('dayRightClick', this, date, true, ev);
	}
...
function Agenda(element, options, methods) {
	...
	function renderAgenda(c, colFormat) {
		...
		if (!head) { // first time rendering, build from scratch
			...
			head.find('td').bind('contextmenu',slotRightClick);
			...
			body.find('td').bind('contextmenu',slotRightClick);
	...
	function slotRightClick(ev) {
		...
		if (rowMatch) {
			...
			date.setHours(hours);
			date.setMinutes(mins%60 + minMinute);
			view.trigger('dayRightClick', this, date, false, ev);
		}else{
			view.trigger('dayRightClick', this, date, true, ev);
		}
	}
	...
	function draggableSlotEvent(event, eventElement, timeElement) {
		if (!options.disableDragging && eventElement.draggable) {
			...
			eventElement.draggable({
				...
				opacity: view.option('dragOpacity'),
				revertDuration: options.dragRevertDuration,
				containment: '.fc-agenda-body:visible table',
				start: function(ev, ui) {
					view.hideEvents(event, eventElement);
					view.trigger('eventDragStart', eventElement, event, ev, ui);
					if ($.browser.msie) {
	...
	eventElementHandlers: function(event, eventElement) {
		...
		eventElement
			...
			.bind('contextmenu',function(ev) {
				if (!eventElement.hasClass('ui-draggable-dragging') &&
					!eventElement.hasClass('ui-resizable-resizing')) {
						return view.trigger('eventRightClick', this, event, ev);
					}
			})
			.hover(
				function(ev) {
